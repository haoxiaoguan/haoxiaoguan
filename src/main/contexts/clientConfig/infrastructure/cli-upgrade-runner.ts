...
// 客户端 CLI 一键升级（对称移植 cc-switch run_tool_lifecycle_silently）：
// 经用户登录 shell 静默跑升级命令（与版本探测同一 PATH 解析路径，确保 npm/node 在 PATH），
// 阻塞到命令结束，无可见终端。优先官方自更新子命令、`||` 回退 npm/pip 全局安装
// （官方子命令能正确处理「非 npm 装」的情形，如 claude 的原生安装器）。
// 失败回传 stderr/stdout 末尾若干行供 toast，绝不宽泛杀进程（只 spawn 我们起的子进程）。

const execFileAsync = promisify(execFile)
const UPGRADE_TIMEOUT_MS = 180_000 // 全局安装可能拉包数十秒~数分钟
...
/** 升级某客户端 CLI。Windows 暂不支持（best-effort：直接报错，让 UI 提示手动升级）。 */
export async function runUpgrade(clientId: ClientId): Promise<UpgradeResult> {
  const command = UPGRADE_SHELL_COMMAND[clientId]
  ...
  try {
    await execFileAsync(shell, [defaultShellFlag(shell), command], { timeout: UPGRADE_TIMEOUT_MS })
    return { ok: true }
  } catch (e) {
