...
//
// The source re-exports AgentId as PlatformId (17 variants, snake_case strings)
// and AgentFamily as IdeFamily. We mirror that exactly by re-exporting the
// agents-layer AgentId so quota code routes per-platform without redefining the
// enum (do NOT redefine — the shared agents layer owns it).
 ...
 * (the 5 CLI-only agents resolve to Unsupported in the fetcher). Mirrors the
 * source AgentId routing in HttpLiveQuotaFetcher.
 */
