...
// ══════════════════════════════════════════════════════════
// QUOTA（402 额度耗尽）：冷却该账号到下一次配额重置时间，期间不再选它
// ══════════════════════════════════════════════════════════
...
describe('QUOTA 402 额度耗尽', () => {
  function quotaDeps(opts: {
    now: { t: number }
    rows: any[]
    chat: (ctx: any) => Promise<CanonicalResponse>
    quotaReset?: { resetAtForAccount: (id: string) => Promise<number | undefined> }
    sleep?: (ms: number) => Promise<void>
  }) {
    const { now, rows, chat } = opts
    const health = new AccountHealthTracker({
      baseCooldownMs: 1000,
      maxBackoffMultiplier: 64,
      quotaResetMs: 3600000,
    ...
    const inner = {
      ...
      supportsModel: () => true,
      listModels: () => [],
      classifyError: (e: any) => (e?.name === 'QuotaErr' ? 'QUOTA' : 'SERVER'),
      chat: (_ir: any, ctx: any) => chat(ctx),
      chatStream: () => {
        throw new Error('n/a')
      },
    }
    ...
    const fa = new FailoverAdapter({
      ...
      sleep: opts.sleep ?? (() => Promise.resolve()),
      random: () => 0.5,
      ...(opts.quotaReset ? { quotaReset: opts.quotaReset } : {}),
    })
    return { fa, health }
  }

  it('用 resolver 的 resetAt 硬冷却该账号到重置时间，期间不可用、到点恢复', async () => {
    const now = { t: 1000 }
    const { fa, health } = quotaDeps({
      now,
      rows: [{ id: 'a', isActive: true }],
      chat: async () => {
        throw new QuotaErr()
      },
      quotaReset: { resetAtForAccount: async () => 5000 },
    })
    await expect(fa.chat(ir, { requestId: 'r' })).rejects.toBeTruthy()
  ...
  it('a 额度耗尽 → 立即切到健康账号 b', async () => {
    ...
    const { fa, health } = quotaDeps({
      ...
      chat: async (ctx) => {
        if (ctx.account.id === 'a') throw new QuotaErr()
        return okResp
      },
      quotaReset: { resetAtForAccount: async () => 999999 },
    })
    const obs: RequestObservation = { attempts: 0 }
    const resp = await fa.chat(ir, { requestId: 'r', observation: obs })
    expect(resp.content[0]).toMatchObject({ text: 'ok' })
  ...
  it('未注入 quotaReset → 退回默认 quotaResetMs(1h) 冷却', async () => {
    const now = { t: 0 }
    const { fa, health } = quotaDeps({
      now,
      rows: [{ id: 'a', isActive: true }],
  ...
  it('resolver 抛错 → 退回默认冷却（不阻断切号）', async () => {
    const now = { t: 0 }
    const { fa, health } = quotaDeps({
      now,
      rows: [{ id: 'a', isActive: true }],
  ...
  it('QUOTA 不触发 sleep（立即切号，区别于 SERVER）', async () => {
    const now = { t: 0 }
...
// ══════════════════════════════════════════════════════════
// DEPOOL（401/403 token 永久失效）：移出反代池 + 立即切号
// ══════════════════════════════════════════════════════════
class TokenDeadErr extends Error {
  constructor() {
    super('token permanently invalid')
    this.name = 'TokenDeadErr'
  }
}

describe('DEPOOL token 永久失效移池', () => {
  function depoolDeps(opts: {
    now: { t: number }
    rows: any[]
    chat: (ctx: any) => Promise<CanonicalResponse>
    ...
    const inner = {
      ...
      supportsModel: () => true,
      listModels: () => [],
      classifyError: (e: any) => (e?.name === 'TokenDeadErr' ? 'DEPOOL' : 'SERVER'),
      chat: (_ir: any, ctx: any) => chat(ctx),
      chatStream: () => {
        throw new Error('n/a')
      },
    ...
    return new FailoverAdapter({
      ...
      random: () => 0.5,
      ...(opts.removeFromPool ? { removeFromPool: opts.removeFromPool } : {}),
    })
  }

  it('token 永久失效 → 调 removeFromPool(id) 移出反代池，并切到健康账号', async () => {
    const now = { t: 0 }
    ...
    const fa = depoolDeps({
      ...
      chat: async (ctx) => {
        if (ctx.account.id === 'a') throw new TokenDeadErr()
        return okResp
      },
    ...
    expect(resp.content[0]).toMatchObject({ text: 'ok' })
    expect(obs.accountId).toBe('b')
    expect(removed).toEqual(['a']) // a 被移出池
  })

  it('removeFromPool 抛错不阻断切号（仍切到 b）', async () => {
    const now = { t: 0 }
    const fa = depoolDeps({
      now,
      rows: [
  ...
  it('未注入 removeFromPool → 不报错（仅熔断，跳过移池）', async () => {
    const now = { t: 0 }
    const fa = depoolDeps({
      now,
      rows: [{ id: 'a', isActive: true }],
      chat: async () => {
        throw new TokenDeadErr()
      },
    })
    await expect(fa.chat(ir, { requestId: 'r' })).rejects.toBeTruthy()
  })
